...
async function main() {
  const holdingsRows = parseCSV(readFileSync("data/holdings.csv", "utf-8"));
  const monitoredRows = existsSync("data/monitored.csv")
    ? parseCSV(readFileSync("data/monitored.csv", "utf-8"))
    : [];
  const rows = [...holdingsRows, ...monitoredRows];
  ...
  if (!symbols.length) {
    console.error("No symbols in holdings.csv or monitored.csv");
    process.exit(1);
  }

  // Discover non-USD currencies across both stock CSVs + map to Yahoo FX symbols.
  // Bundling FX into the same batch keeps this cron at one HTTP call total.
  const currencies = [...new Set(rows.map((r) => (r.currency || "USD").trim()))]
    .filter((c) => c && c !== "USD");
