...
const state = {
  rows: [],          // main holdings, filtered by owner and country
  cash: [],          // { date, amount, currency, usdRate, amountUsd, owner }
  sort: "value",
  owner: "All",
  country: "All",
  overviewOpen: false,
...
function stockDetailUrl(symbol, owner) {
  const query = new URLSearchParams({ symbol, owner });
...
function renderRow(row) {
  ...
  quoteBtn.href = stockDetailUrl(holding.symbol, holding.owner);
...
function filteredRows() {
  return rowsForScope(state.rows, state.owner, state.country);
}

function filteredCash() {
  return cashForScope(state.cash, state.owner, state.country);
}
...
function consolidateCash(entries) {
  ...
  for (const c of entries) {
    if (!isFinite(c.amountUsd)) continue;
...
function renderHoldings() {
  ...
  if (!sorted.length && !cashRows.length) {
    ...
    const ownerText = state.owner === ALL ? "" : ` for ${state.owner}`;
    const countryText = state.country === ALL ? "" : ` in ${state.country}`;
    empty.textContent = `No holdings found${ownerText}${countryText}.`;
...
function renderTabs() {
  ...
  for (const r of state.rows) {
    ...
    const value = isFinite(usd) ? usd : 0;
    totalUsd += value;
    ownerTotals.set(
      r.holding.owner,
      (ownerTotals.get(r.holding.owner) || 0) + value,
    );
  }

  for (const c of state.cash) {
    const value = isFinite(c.amountUsd) ? c.amountUsd : 0;
    totalUsd += value;
    ownerTotals.set(c.owner, (ownerTotals.get(c.owner) || 0) + value);
  }

  const owners = ownersForPortfolio(state.rows, state.cash);
  const tabs = [ALL, ...owners];

  const tabsEl = $("tabs");
  tabsEl.innerHTML = "";
  for (const t of tabs) {
    const btn = document.createElement("button");
    btn.className = "tab" + (state.owner === t ? " active" : "");
    btn.dataset.owner = t;
    ...
    value.textContent = fmtMoney(t === ALL ? totalUsd : ownerTotals.get(t));
...
function renderCountryFilter() {
  ...
  const countries = countriesForOwner(state.rows, state.owner);
...
function renderSummary() {
  ...
  if (labelEl) {
    const scope = [
      state.owner === ALL ? null : state.owner,
      state.country === ALL ? null : state.country,
    ].filter(Boolean);
    labelEl.textContent = `Portfolio value${scope.length ? ` · ${scope.join(" · ")}` : ""}`;
  }
...
function renderOverview() {
  ...
  const tabName = [
    state.owner === ALL ? "All positions" : state.owner,
    state.country === ALL ? null : state.country,
  ].filter(Boolean).join(" · ");
  const header =
...
async function loadAllLive(force = true) {
  ...
  try {
    ...
    // Batch all stock symbols + FX symbols through Yahoo Finance in one call.
    // Extended-hours prices come back for stocks
...
async function loadAllParse() {
  setParsing(true);
  try {
    const stockSymbols = [...new Set(state.rows.map((r) => r.holding.symbol))];
    const fxSyms = fxSymbolsForCurrencies(uniqueCurrencies());
...
function bindRowNavigation() {
  ...
  list.addEventListener("click", (e) => {
    ...
    window.location.href = stockDetailUrl(row.dataset.symbol, row.dataset.owner);
  });
...
async function main() {
  ...
  if (!holdings.length && !cash.length) {
    $("empty").hidden = false;
