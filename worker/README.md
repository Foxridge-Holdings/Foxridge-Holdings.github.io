...
`https://foxridge-holdings-yahoo-proxy.<account>.workers.dev`. Paste it into the
...
```sh
curl 'https://foxridge-holdings-yahoo-proxy.<account>.workers.dev/?symbol=AAPL' | head -c 200
# → {"chart":{"result":[{"meta":{...

curl -I -H 'Origin: https://foxridge-holdings.github.io' \
  'https://foxridge-holdings-yahoo-proxy.<account>.workers.dev/?symbol=AAPL'
